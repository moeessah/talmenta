...
const setTheme=(theme,save=true)=>{
  ...
  if(save){try{localStorage.setItem('talmenta-theme',next);}catch(e){}}
  updateThemeControl();
  observeHeaderTheme();
};
updateThemeControl();
...
if(contactForm){
  contactForm.addEventListener('submit',e=>{
    ...
    if(!email)return;
    window.location.href='mailto:hello@talmenta.org?subject='+encodeURIComponent('Research problem')+'&body='+encodeURIComponent('I would like to talk about a research problem. My email is '+email+'.');
    input.value='';
  });
}
})();
